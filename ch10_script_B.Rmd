...
average_start <- train %>%
  summarize(average = mean(att_start))

average_end <- train %>%
  summarize (average = mean(att_end))

average_change <- train %>% 
  summarize (average = mean(att_chg))
```

``` {r scene 8 part two, include = F}
...
# Frequentist - sample an infinite number of times (hypothetical repetition/objective?)

# Bayesian - probability only based on the information you have (subjective/prior belief)

lower_prob = quantile(g_samples, (1-0.99/2))

upper_prob = quantile(g_samples, level + (1-level)/2)

treat_cong <- diff %>%
  pull(average) %>%
  quantile(c(((1-0.99)/2), ().99 + (1-0.99)/2))
...
train %>%
  ...
  pivot_wider(id_cols = replicat, names_from = treatment, values_from = avg_att_chg) %>%
  mutate(avg_chg_diff = Treated - Control) %>%
  ungroup() %>%
  summarize(
    lower = quantile(att_chg_diff = c(0.005)), 
    median = quantile(att_chg_diff = c(0.5)), 
    upper = quantile(att_chg_diff = c(0.995))
  )
